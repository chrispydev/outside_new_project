...
class Register extends React.Component {
  ...
  getInfo = async () => {
    ...
    const resmodule = await fetch('http://localhost:5000/api/modules');
    if (res.status !== 200) {
  ...
  submitForm = (e) => {
    ...
    `)
      .then((res) => {
        res.status === 200
          ? this.notification('Registered Successfull', 'success')
          : this.notification('Registered Failed', 'error');
      })
  ...
  componentDidMount() {
    this.getInfo().then((res) => {
      console.log(res.datamodule);
      this.setState({
        universityList: res.data,
        modulesList: res.datamodule,
      });
    });
  }
        ...
        <div className='bg-light text-dark'>
          <div className='container'>
            <div className='d-flex justify-content-center'>
              <p className='mt-4 text-uppercase font-weight-bold h2'>
                Register Now
              </p>
            </div>
                  ...
                  <div className='form-group col-md-4 '>
                    ...
                    >
                      ...
                      {this.state.modulesList.map((mod) => {
                        return (
                          <option key={mod.m_id} value={mod.m_id}>
                            {mod.m_name}
                          </option>
                        );
                      })}
